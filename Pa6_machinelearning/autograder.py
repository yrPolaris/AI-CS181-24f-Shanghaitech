...
def check_attention(tracker):
    ...
    input_params = np.load('data/input.npz')
    input_data = input_params['input']

    att_block.k_weight = input_params['k_weight']
    att_block.q_weight = input_params['q_weight']
    att_block.v_weight = input_params['v_weight']
    output = att_block.forward(input_data)
    expected_output = np.load("data/output.npz")['output']
    # Check if the output from the Attention block is close to the expected output
    assert np.allclose(output, expected_output), (
        f"Test failed for block_size={block_size}, layer_size={layer_size}. "
        f"Output: {output} does not match expected output: {expected_output}"
    )
