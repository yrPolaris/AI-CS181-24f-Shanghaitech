    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        mostLikelyPos = [max(beliefDist, key=beliefDist.get) for beliefDist in livingGhostPositionDistributions]
        bestAction = min(legal, key=lambda action: min(
            self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghostPos)
