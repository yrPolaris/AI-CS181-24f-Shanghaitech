...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            food_remain = state[1].copy()
            if (nextx, nexty) in food_remain:
                food_remain.remove((nextx, nexty))
            successors.append(([(nextx, nexty), food_remain], action, 1))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    def set_union(item_i : Tuple[int, int], item_j : Tuple[int, int]):
        find_i = find(item_i)
        find_j = find(item_j)
        if find_i != find_j:
    ...
    for food in food_list:
        food_heu = min(food_heu, mazeDistance(position, food, problem.startingGameState))
        parent[food] = food
        
    mst_edges : List[Tuple[Tuple[int, int], Tuple[int, int], int]] = []
    for i in food_list:
        for j in food_list:
            if i == j:
                continue
    ...
    while num_edges < needed_edges:
        ...
        if food_a_parent != food_b_parent:
            num_edges += 1
            mst_heu += dis
            set_union(food_a, food_b)
    return food_heu + mst_heu
