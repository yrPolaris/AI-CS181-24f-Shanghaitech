...
from typing import Dict, List, Tuple, Callable, Generator, Any, Union
import util
import sys
import logic
import game
...
def sentence2() -> Expr:
    ...
    Exp1: Expr = C % (B | D)  # C if and only if (B or D)
    Exp2: Expr = A >> ((~B) & (~D))  # A implies ((not B) and (not D))
    Exp3: Expr = (~(B & (~C))) >> A  # (not (B and (not C))) implies A
    Exp4: Expr = (~D) >> C  # (not D) implies C
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PacmanAlive0: PropSymbolExpr = PropSymbolExpr('PacmanAlive', time=0)
    PacmanAlive1: PropSymbolExpr = PropSymbolExpr('PacmanAlive', time=1)
    PacmanKilled0: PropSymbolExpr = PropSymbolExpr('PacmanKilled', time=0)
    PacmanBorn0: PropSymbolExpr = PropSymbolExpr('PacmanBorn', time=0)
    
    Exp1: PropSymbolExpr = PacmanAlive1 % ((PacmanAlive0 & (~PacmanKilled0)) | (~PacmanAlive0 & PacmanBorn0))
    Exp2: PropSymbolExpr = ~(PacmanAlive0 & PacmanBorn0)
    Exp3: PropSymbolExpr = PacmanBorn0
    return conjoin([Exp1, Exp2, Exp3])
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    clauses: List[Expr] = []
    for i1, i2 in itertools.combinations(literals, 2):
        clause = (~i1 | ~i2)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            PositionExpList.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(PositionExpList))

        CurrModel: Union[Dict[Expr, bool]] = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time=t)]))
        if CurrModel:
            return extractActionSequence(CurrModel, actions=actions)
    
        ActionExpList = []
        for action in actions:
            ActionExpList.append(PropSymbolExpr(action, time=t))
        KB.append(exactlyOne(ActionExpList))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        FoodExp: List[PropSymbolExpr] = []
        for x, y in food:
            FoodExp.append(PropSymbolExpr(food_str, x, y, time=t))
            KB.append(PropSymbolExpr(food_str, x, y, time=t + 1) % (PropSymbolExpr(food_str, x, y, time=t) & (~PropSymbolExpr(pacman_str, x, y, time=t))))
    
        CurrModel: Union[Dict[Expr, bool]] = findModel(conjoin(KB + [~ disjoin(FoodExp)]))
        if CurrModel:
            return extractActionSequence(CurrModel, actions=actions)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=walls_grid, sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))

        possible_locations: List[Tuple[int, int]] = []
        for x, y in non_outer_wall_coords:
            AtLocationXY: bool = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t))
            NotLocationXY: bool = entails(conjoin(KB), (~PropSymbolExpr(pacman_str, x, y, time=t)))
            
            if not NotLocationXY:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=known_map, sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))

        for x, y in non_outer_wall_coords:
            WallAtLocationXY: bool = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            NotWallAtLocationXY: bool = entails(conjoin(KB), (~PropSymbolExpr(wall_str, x, y)))
        
            if WallAtLocationXY:
                KB.append(PropSymbolExpr(wall_str, x, y))
                known_map[x][y] = 1        
            if NotWallAtLocationXY:
                KB.append(~PropSymbolExpr(wall_str, x, y))
                known_map[x][y] = 0
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=known_map, sensorModel=SLAMSensorAxioms, successorAxioms=SLAMSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
