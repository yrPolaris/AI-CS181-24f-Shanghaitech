...
def update_Q_table(reward, next_state, state, Q_table, action):
    ...
    Q_table[0, q_index[1], q_index[2], action] += \
        alpha * (reward + gamma * np.max(Q_table[0, next_q_index[1], next_q_index[2]]) \
            - Q_table[0, q_index[1], q_index[2], action])
    print(q_index[1],q_index[2],Q_table[0, q_index[1], q_index[2], 0],Q_table[0, q_index[1], q_index[2], 1],Q_table[0, q_index[1], q_index[2], 2])
    return Q_table

def epsilon_greedy(state, epsilon):
    q_index = state_to_q_index(state)
    # best_action = np.argmax(Q_table[q_index[0], q_index[1], q_index[2]])
    best_action = np.argmax(Q_table[0, q_index[1], q_index[2]])
...
for episode in range(episodes):
    ...
    for t in range(max_timesteps):
        ...
        #连续的后退只给第一次后退一个reward
        if((action == 1 or action == 0) and reward_y < 0 and pre_reward_y >= 0):
            reward_y *= 100
        if(reward == 1):
            reward_y = 0
        pre_info_y = info['labels']['player_y']
        pre_reward_y = reward_y
